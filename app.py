...
DB_PATH = "data.db"
APP_URL = "https://neardoer.streamlit.app"  # update if different
...
def fetch_tasks(status: str = None, zip_filter: str = None, category: str = None):
    ...
    cur = conn.cursor()
    query = """
        SELECT id, title, description, category, price, zip, status, 
               posted_by, accepted_by, created_at, updated_at 
...
def get_or_create_user(name: str, role: str, zip_code: str, skills: str = "") -> int:
    conn = get_conn()
    cur = conn.cursor()
    cur.execute("SELECT id FROM users WHERE name=? AND role=? AND zip=?", 
                (name.strip(), role.strip(), zip_code.strip()))
    row = cur.fetchone()
    if row:
        user_id = row[0]
        cur.execute("UPDATE users SET skills=? WHERE id=?", (skills.strip(), user_id))
    else:
        cur.execute("INSERT INTO users (name, role, zip, skills) VALUES (?, ?, ?, ?)",
                    (name.strip(), role.strip(), zip_code.strip(), skills.strip()))
        conn.commit()
...
def accept_task(task_id: int, helper_id: int):
    ...
    conn = get_conn()
    cur = conn.cursor()
    cur.execute("UPDATE tasks SET status='Accepted', accepted_by=?, updated_at=? WHERE id=? AND status='Open'",
...
def complete_task(task_id: int):
    ...
    conn = get_conn()
    cur = conn.cursor()
    cur.execute("UPDATE tasks SET status='Completed', updated_at=? WHERE id=? AND status='Accepted'",
                (now, task_id))
    conn.commit()
    conn.close()
...
.hero {background: linear-gradient(90deg,#2563EB,#7C3AED);color:white;
padding:14px;border-radius:12px;margin:6px 0 12px 0;
display:flex;align-items:center;gap:12px;}
.hero .logo {width:40px;height:40px;border-radius:50%;background:#fff2;
display:flex;align-items:center;justify-content:center;font-weight:800;}
.card {border:1px solid #eaeef4;border-radius:10px;padding:10px 12px;
margin-bottom:8px;background:#fff;}
</style>
<div class="hero"><div class="logo">ND</div>
<div><b>NearDoer — Get Small Things Done Fast</b><br>
<span style="font-size:13px">Post · AI ranks · Accept · Done</span></div></div>
...
if st.sidebar.button("Save / Switch Profile"):
    if name and zip_code:
        ...
        fetch_tasks.clear()
        st.sidebar.success(f"Profile saved as {role} (ID {uid}).")
            ...
            if pid==user["id"]:
                ...
                if stt=="Accepted":
                    if st.button("Mark Completed", key=f"c{tid}"):
                        complete_task(tid); fetch_tasks.clear(); st.success("Task marked completed!")
...
# Helper
    ...
    with col1:
        ...
        ranked = rank_tasks_by_match(open_tasks, user.get("skills",""))
        for (task,sc) in ranked:
            tid,tit,desc,cat,pr,zp,stt,pid,aid,_,_ = task
            st.markdown(f"**{tit}** (AI {sc:.2f}) - {desc}")
